...
BEGIN
    ...
    VALUES (
        ...
        @PartnerNumber+ @i, -- PartnerNumber
        @CroatianPIN + @i, -- CroatianPIN
        CAST(RAND() * 2 + 1 AS INT), -- PartnerTypeId
        GETUTCDATE(), --CreatedAtUtc
        'user@example.com', -- CreateByUser
        CAST(RAND() * 1 AS BIT), -- IsForeign
        CAST(CAST(RAND() * 8999999999999999999 + 1000000000000000000 AS BIGINT)AS NVARCHAR(20)),	-- ExternalCode
...
BEGIN
	...
	WHILE @j < 10
    BEGIN
		INSERT INTO [dbo].[Policy] ([ShelfNumber], [PolicyAmount], [PartnerId])
		VALUES (
